    ...
    return (
        <Dialog open={open} onOpenChange={onClose}>
            <DialogContent>
                ...
                <div className="space-y-4">
                    <p className="text-sm text-gray-600">
                        Ban co chac muon dat lai mat khau cho <strong>{user?.full_name || user?.email}</strong>?
                    </p>
                    <p className="text-sm text-amber-600 bg-amber-50 p-3 rounded-lg">
                        Mat khau moi se duoc tao tu dong va gui ve email cua nguoi dung.
                    </p>
                </div>
...
export default function UserManagement() {
    ...
    const resetPasswordMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            if (data.data?.new_password) {
                toast.success(`Mat khau moi: ${data.data.new_password}`, { duration: 10000 })
            }
            closeDialogs()
        },
        onError: (error) => {
    ...
    const deleteUserMutation = useMutation({
        ...
        onSuccess: (data) => {
            queryClient.invalidateQueries(['admin', 'users'])
            toast.success(data.message || 'Da xoa nguoi dung thanh cong')
        },
        onError: (error) => {
            toast.error(error.response?.data?.message || 'Co loi xay ra khi xoa nguoi dung')
        }
    })
    ...
    const handleLockUser = (reason, sendEmail) => {
        if (!selectedUser?.id) {
            toast.error('Vui long chon nguoi dung')
            return
        }
    ...
    const handleDeleteUser = (user) => {
        ...
        if (window.confirm(`Ban co chac muon xoa nguoi dung ${user.full_name || user.email}? Hanh dong nay khong the hoan tac.`)) {
